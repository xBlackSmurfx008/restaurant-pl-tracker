  ...
  const loadData = useCallback(async () => {
    try {
      ...
      const [expensesData, categoriesData, vendorsData, summaryData] = await Promise.all([
        api.request(`/expenses?start_date=${filters.start_date}&end_date=${filters.end_date}${filters.expense_type ? `&expense_type=${filters.expense_type}` : ''}${filters.category_id ? `&category_id=${filters.category_id}` : ''}`),
        api.request('/expenses/categories/grouped'),
        api.getVendors(),
        api.request(`/expenses/summary?start_date=${filters.start_date}&end_date=${filters.end_date}&group_by=type`)
      ]);
